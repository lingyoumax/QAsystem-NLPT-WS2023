...
def get_top_abstracts(query, bm25, model, top_n=10):
    ...
    for doc_index, document in enumerate(bm25.corpus):
        ...
        query_features.append([bm25_score, doc_len, common_terms_count, common_terms_ratio,avg_df, avg_idf])  # , pub_year, title_len
...
for query, correct_pmid in tqdm(data_list, desc="Processing queries"):
    top_pmids = get_top_abstracts(query, bm25, lr_model)
