...
class BERTMultiLabelBinaryClassification_FactorLoss(torch.nn.Module):
    def __init__(self, num_labels, label_weight, gamma=2.0,mix_ratio=0.5):
        ...
        self.gamma = gamma
        self.mixed_loss = MixedLoss(weight=self.label_weight, gamma=self.gamma,pos_weight=self.label_weight,mix_ratio=mix_ratio).to(device)
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        if labels is not None:
            loss = self.mixed_loss(logits.view(-1, self.bert.config.num_labels), labels.view(-1, self.bert.config.num_labels))
            return loss
        else:
            return logits
